...
- Backend: Java 17, Spring Boot 3.4.x, `jakarta.*`, API DTOs (`StudentRequest` / `StudentResponse`) — not the JPA entity as the HTTP contract; OpenAPI via springdoc → committed `api/openapi.json`; structured errors with `ApiErrorCode`
