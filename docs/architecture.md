...
| Errors | `apiError` maps known OpenAPI `ApiErrorCode` values to i18n (`errors.codes.*`); unknown codes / unexpected failures → generic i18n; `message` is diagnostic only for known codes; list load failures render in-page error + Retry |
